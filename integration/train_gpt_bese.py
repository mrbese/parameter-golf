...
def eval_val_slot(
    ...
    for batch_idx in range(0, len(my_windows), batch_seqs):
        ...
        inputs = torch.zeros(bsz, seq_len, dtype=torch.int64, device=device)
        targets = torch.zeros(bsz, seq_len, dtype=torch.int64, device=device)
        wlens: list[int] = []
        ...
        for i, ws in enumerate(batch_ws):
            ...
            chunk = val_tokens[ws:end + 1].to(dtype=torch.int64, device=device)
            inputs[i, :wlen] = chunk[:-1]
            targets[i, :wlen] = chunk[1:]

        with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            hidden = compiled_hidden(inputs)
        ...
        for i in range(bsz):
            mask[i, score_starts[i]:wlens[i]] = 1.0
        ...
        with torch.no_grad():
            ...
            for i in range(bsz):
                s = score_starts[i]
                wlen = wlens[i]
                scored = per_token[i, s:wlen].to(torch.float64)
                loss_sum += scored.sum()
                token_count += float(wlen - s)
                tgt = targets[i, s:wlen]
                prev = inputs[i, s:wlen]
                tb = base_bytes_lut[tgt].to(torch.float64)
                tb += (has_leading_space_lut[tgt] & ~is_boundary_token_lut[prev]).to(torch.float64)
                byte_count += tb.sum()
