#!/usr/bin/env python3
"""
RunPod v6.1: End-to-end BESE submission pipeline for Parameter Golf.

Targets 8xH100 pod. Runs the full v6.1 pipeline:
...
v6.1 changes vs v6:
  - 12 layers (was 13) — faster steps, more total steps in 600s
  - MLP_MULT=3.5 (was 3) — wider MLP, fills 16 MB budget
  - PARALLEL_RESIDUAL_START=8 (was 9) — last 4 of 12 layers
  - VE_LAYERS=10,11 (was 11,12) — last 2 of 12 layers
  - LZMA preset 9 (was 6) — better compression for size headroom
  - Legal TTT at eval time — score-first SGD for ~0.02-0.03 BPB improvement

Usage (on the RunPod pod):
  cd /workspace && git clone https://github.com/mrbese/parameter-golf-bese.git bese
  cd /workspace/bese && python scripts/runpod_v6_1.py

  # Resume with existing shards (rebuilds ngram table):
  python scripts/runpod_v6_1.py --skip-shards

  # Resume after data prep:
  python scripts/runpod_v6_1.py --skip-prep

  # Resume after training:
  python scripts/runpod_v6_1.py --skip-prep --skip-train
"""
...
NGRAM_TABLE = BESE_DIR / "artifacts" / "ngram_table_v6_1.bin"
TRAIN_SCRIPT = BESE_DIR / "integration" / "train_gpt_bese.py"
LOGFILE = WORK_DIR / "run_v6_1.log"

# ---------------------------------------------------------------------------
# Training environment (v6.1 target configuration)
# ---------------------------------------------------------------------------
TRAIN_ENV = {
    "VOCAB_SIZE": "288",
    "NUM_LAYERS": "12",                          # v6.1: was 13 — faster steps, more total steps
    "MODEL_DIM": "512",
    "MLP_MULT": "3.5",                           # v6.1: was 3 — wider MLP, fills 16 MB budget
    "NUM_HEADS": "8",
    "NUM_KV_HEADS": "4",
    ...
    "PARALLEL_RESIDUAL_START": "8",              # v6.1: was 9 — last 4 of 12 layers (8,9,10,11)
    "QK_GAIN_INIT": "5.25",
    "MATRIX_LR": "0.026",
    "MUON_WD": "0.095",
    "ADAM_WD": "0.095",
    "EMA_DECAY": "0.9965",
    "WARMDOWN_ITERS": "5000",
    "VE_LAYERS": "10,11",                        # v6.1: last 2 of 12 layers (indices 0-11)
    "EVAL_STRIDE": "64",
    ...
    "TTT_ENABLED": "1",                          # v6.1: NEW — enable Legal TTT at eval time
    "TTT_LR": "0.005",                           # v6.1: NEW — SGD learning rate for TTT
    "TTT_MOMENTUM": "0.9",                       # v6.1: NEW — SGD momentum
    "TTT_EPOCHS": "3",                           # v6.1: NEW — epochs per chunk
    "TTT_GRAD_CLIP": "1.0",                      # v6.1: NEW — gradient clipping
}
    ...
    for line in output.strip().split("\n"):
        if "final_ttt_sliding_window_exact" in line and "val_bpb:" in line:
            for part in line.split():
                ...
                if part.startswith("val_loss:"):
                    metrics["ttt_loss"] = float(part.split(":")[1])
...
def phase0_data_prep() -> None:
    ...
    if BPE_OUTPUT.exists():
        ...
        t_bpe = time.time()
        # Extract just the text from scored_train for BPE training (first 50K docs)
        bpe_sample = [doc for _, doc in scored_train[:50000]]
        merges = train_bpe_merges_fast(bpe_sample, num_merges=248, verbose=True)
        del bpe_sample
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="BESE v6.1: Full Parameter Golf submission pipeline (8xH100)"
    )
    parser.add_argument(
        "--skip-prep",
        action="store_true",
